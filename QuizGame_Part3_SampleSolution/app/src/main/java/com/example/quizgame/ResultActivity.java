...
public class ResultActivity extends AppCompatActivity {
    ...
    private void linkViews() {
        ...
        /* Part 3: The result screen should tell the player that they won, lost, cheated,
        ...
        switch (currentGame.currentState) {
            case WIN: // if (currentState == WIN)
                ...
                resultText.setText("Congrats! You're smarter than a 5th grader!");
                break;
            case LOSE: // else if (currentState == LOSE)
                /* What message should be displayed when the player loses the game?
                 * To let the player know the correct answer to the last question they answered,
                 * a sample answer can be included in this message, provided below as the
                 * variable "actualAnswer". */
                String actualAnswer = CurrentGame.getCurrentQuestion().answers[0];
                resultText.setText("Sorry, you're not smarter than a 5th grader :(\n The actual answer is \"" + actualAnswer + "\".");
                break;
            case CHEAT: // else if (currentState == CHEAT)
                /* What message should be displayed when the player cheats in the game? */
                resultText.setText("A cheater is definitely not smarter than a 5th grader >:(");
                break;
            default: // else
                /* What message should be displayed when the player's status is none of the above? */
                resultText.setText("Could be smarter than a 5th grader...");
                break;
        }
    }
